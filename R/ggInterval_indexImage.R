    ...
    if (plotAll) {
      ...
      for (i in colnames(iData)) {
        ...
        d <- rbind(d, data.frame(
          buildPlotData(a, b, adjustStrip, n, full_strip, datasetMax),
          group = factor(i)
        ))
      }
    } else{
...
buildPlotData <- function(a,
  ...
  if (!full_strip) {
    ...
    adjustCoef <- ifelse(datasetMax * n > 2000, 1, ceiling(2000 / (datasetMax *
                                                                     n)))
    
    myAdjust <- seq(1, datasetMax, (datasetMax - 1) / (datasetMax * adjustCoef -
                                                         1))
    #debug for unequal bin image
    d2 <- data.frame(
      x = rep(1:n, each = datasetMax * adjustCoef) - 0.5,
    ...
    val2 <- mapply(
      ...
      b,
      FUN = function(x, y)
        sort(runif(datasetMax * adjustCoef, x, y))
    )

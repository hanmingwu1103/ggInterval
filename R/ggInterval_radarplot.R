...
ggInterval_radarplot <- function(data = NULL,
  ...
  if (is.null(plotPartial)) {
    ...
    if (!is.numeric(plotPartial)) {
      stop(
        "ERROR IN plotPartial : the input data must be numeric vector as the row index in full data."
      )
    }
  ...
  if (!(type %in% c("default", "rect", "quantile"))) {
    warning(paste(
      ...
      type,
      ", automatically set default type."
    ))
    type <- "default"
  }
  ...
  if (nP != 0) {
    ...
    minDF <- sapply(
      1:nP,
      FUN = function(x)
        matrix(normData[[x]], ncol = 2)[, 1]
    )
    maxDF <- sapply(
      1:nP,
      FUN = function(x)
        matrix(normData[[x]], ncol = 2)[, 2]
    )
    ...
    temp <- lapply(
      1:nP,
      FUN = function(x)
        paste(iDataList[[x]][, 1], iDataList[[x]][, 2], sep = ":")
    )
    temp <- lapply(
      1:nP,
      FUN = function(x)
    ...
    textMin$cos <- textMin$cos + textShift
    textMin$sin <- textMin$sin + textShift
    textMax$cos <- textMax$cos + textShift
    textMax$sin <- textMax$sin + textShift
  ...
  if (allnP != nP) {
    ...
    for (var in colnames(propData)) {
      ...
      for (ele in 1:indNum) {
        ...
        propDf[counter:(counter + (length(varPro) / 2) - 1), "varName"] <- rep(var, length(varPro) /
                                                                                 2)
        propDf[counter:(counter + (length(varPro) / 2) - 1), "varLevels"] <-
          paste(varPro[1:(length(varPro) / 2)], round(as.numeric(varPro[(length(varPro) /
                                                                           2 + 1):length(varPro)]), 2), sep = ":")
        propDf[counter:(counter + (length(varPro) / 2) - 1), "prop"] <- as.numeric(varPro[(length(varPro) /
                                                                                             2 + 1):length(varPro)])
        propDf[counter:(counter + (length(varPro) / 2) - 1), "groupid"] <- rep(ele, length(varPro) /
                                                                                 2)
        #extendUnit adjust  quantile(seq(0,1,1/3),seq(0,1,1/3))[2:(3+1)]
        #pos<-seq(0,1,1/((length(varPro)+2)/2))
        pos <- as.numeric(quantile(seq(0, 1 + extendUnit, 1 / ((length(varPro) +
                                                                  2) / 2
        )), seq(0, 1, 1 / ((length(varPro) + 2) / 2
        )))[2:(length(varPro) / 2 + 1)])
        #pos <- pos[-length(pos)][-1]
        #end adjust
        propDf[counter:(counter + (length(varPro) / 2) - 1), "x"] <- d[nP +
                                                                         varNum, 3] * pos
        propDf[counter:(counter + (length(varPro) / 2) - 1), "y"] <- d[nP +
                                                                         varNum, 4] * pos
  ...
  if (type == "rect") {
    if (nP != 0) {
      cutDf <- c(
        getCutDf(plotMin, transMatrix, nP, indNum),
        getCutDf(plotMax, transMatrix, nP, indNum)
      )
    }
  }
  ...
  if (inOneFig) {
      ...
      if (nP != 0) {
        for (i in levels(as.factor(plotMin$group))) {
          ...
          p <- p + geom_path(data = myPathData,
                             aes(x = myPathData$x1, y = myPathData$y1),
                             lty = 0) +
            geom_path(data = myPathData,
                      aes(x = myPathData$x2, y = myPathData$y2),
                      lty = 0)
        }
      }
        ...
        if (type == "quantile") {
          p <- p + geom_polygon(
            ...
            aes(
              x = myPolyData$cos,
              y = myPolyData$sin,
              fill = myPolyData$group,
              col = myPolyData$group
            ),
            alpha = alpha
          )
        } else{
      ...
      if (nP != 0) {
        ...
        for (g in levels(as.factor(cutDf[[1]]$obsGroup))) {
          cutDf.temp <- lapply(
            1:4,
            FUN = function(x) {
              as.data.frame(dplyr::filter(cutDf[[x]], cutDf[[x]]$obsGroup == g))
            }
          )
          #print(cutDf.temp)
          for (i in 1:nP) {
    ...
    #nominal
    if (allnP != nP) {
      newPropDf <- as.data.frame(tidyr::separate(
        propDf,
        varLevels,
        into = c("varLevels", "gar"),
        sep = ":",
        convert = F
      ))
      newPropDf <- as.data.frame(dplyr::filter(
        newPropDf,
        newPropDf$groupid == levels(newPropDf$groupid)[1]
      ))
      ...
      if (addText_modal) {
        p <- p + geom_text(
          data = newPropDf,
          aes(
            x = newPropDf$x + textShift,
            y = newPropDf$y + textShift,
            label = newPropDf$varLevels
          ),
          vjust = 2.15,
          hjust = 0.5,
          size = textSize,
      ...
      if (addText_modal.p) {
        ...
        for (i in unique(totalRectDf$varGroup)) {
          ...
          for (u in levels(temp.varG$obsGroup)) {
            ...
            for (k in unique(temp.obsG$varLevels)) {
              ...
              tempd <- rbind(tempd, data.frame(
                x = mean(temp.varL$newx),
                y = mean(temp.varL$newy)
              ))
            }
          }
        }
        ...
        p <- p + geom_text(
          data = propTextDf,
          aes(
            x = propTextDf$x + textShift,
            y = propTextDf$y,
            label = round(propTextDf$prop, 2)
          ),
          size = textSize,
          col = textCol
        )
    ...
    if (nP != 0) {
      p <- p + geom_point(data = plotMax, aes(
        x = 0,
        y = 0,
        alpha = plotMax$Variables
      )) +
        geom_text(data = newD, aes(
          x = newD$x,
          y = newD$y,
          label = c(colnames(rawiData))
        ))
    ...
    if (!showXYLabs) {
      p <- p + scale_x_continuous(labels = NULL,
                                  limits = c(-xyLimits, xyLimits)) +
        scale_y_continuous(labels = NULL,
                           limits = c(-xyLimits, xyLimits)) + xlab(NULL) + ylab(NULL)
    } else{
      #adds scale y continuous label
      yLabelPos <- seq(-xyLimits, xyLimits, (xyLimits - (-xyLimits)) / (nP +
                                                                          1))
      ...
      yLabels <- paste0(yLabelTitle, paste(unlist(minminList), unlist(maxmaxList), sep =
                                             ","), "]")
      yLabels <- c(yLabels, expression(bold(
        "The range of each variable show as follows :"
      )))
    ...
    if (base_circle) {
      ...
      tmp <- quantile(seq(0 + extendUnit, 1 + extendUnit, 1 / layerNumber),
                      seq(0, 1, 1 / layerNumber))
      #tmp<-seq(0,1,1/layerNumber)[-1]
      #end adjust
      tmp <- data.frame(
        x = a * tmp,
        y = b * tmp,
        label = paste0(as.character(round((tmp - extendUnit) * 100
        )), "%")
      )
      p <- p + geom_text(data = tmp, aes(
        x = tmp$x,
        y = tmp$y,
        label = tmp$label
      ))
    ...
    for (i in levels(as.factor(plotMin$group))) {
      ...
      if (nP == 0) {
        stop(
          "Cannot allows discrete variables only, under the parameter inOneFig = FALSE,
      ...
      if (allnP != nP) {
        ...
        totalRectDf.temp <- dplyr::filter(totalRectDf,
                                          totalRectDf$obsGroup == levels(totalRectDf$obsGroup)[u])
      } else{
        propDf.temp <- 0
        totalRectDf.temp <- 0
      }
      ...
      if (type == "rect") {
        ...
        for (k in 1:4) {
          ...
          cutDf.temp[[k]] <- dplyr::filter(cutDf[[k]], cutDf[[k]]$obsGroup ==
                                             levels(cutDf[[k]]$obsGroup)[u])
        }
      } else{
        cutDf.temp <- 0
      }
      ...
      base <- base + labs(title = paste0("Radar : ", rownames(iData)[u])) +
        labs(title = paste0("Radar : ", rownames(iData)[u])) + scale_colour_discrete(name = "Group") +
    ...
    p <- gridExtra::marrangeGrob(plotList, ncol = myCol, nrow = myRow)
...
plotFun <- function(p,
  ...
  if (allnP != nP) {
    ...
    if (addText_modal.p) {
      ...
      for (i in levels(totalRectDf.temp$varGroup)) {
        temp.varG <- dplyr::filter(totalRectDf.temp, totalRectDf.temp$varGroup ==
                                     i)
      ...
      #print(propTextDf)
      base <- base + geom_text(data = propTextDf,
                               aes(
                                 x = propTextDf$x + textShift,
                                 y = propTextDf$y,
                                 label = round(propTextDf$prop, 2)
  ...
  base <- base + geom_segment(
    ...
    geom_point(data = plotMax.temp, aes(
      x = 0,
      y = 0,
      alpha = plotMax.temp$Variables
    )) +
    geom_text(data = newD, aes(
      x = newD$x,
      y = newD$y,
  ...
  if (addText) {
    base <- base + geom_text(data = textMin.temp,
                             aes(
                               x = textMin.temp$cos,
                               y = textMin.temp$sin,
                               label = textMin.temp$min
                             )) +
      geom_text(data = textMax.temp,
                aes(
                  x = textMax.temp$cos,
                  y = textMax.temp$sin,
                  label = textMax.temp$max
                ))
  }
  
  if (!showXYLabs) {
    base <- base + scale_x_continuous(labels = NULL,
                                      limits = c(-xyLimits, xyLimits)) +
      scale_y_continuous(labels = NULL,
                         limits = c(-xyLimits, xyLimits)) + xlab(NULL) + ylab(NULL)
  } else{
  ...
  if (base_circle) {
    ...
    base <- base + geom_text(data = tmp, aes(
      x = tmp$x,
      y = tmp$y,
      label = tmp$label
    ))
...
data2Vec <- function(iData = NULL,
  ...
  if (type == "quantile") {
    for (i in 1:dim(data)[1]) {
      result <- rbind(result, data.frame(data[i, ] * transMat, group = paste(
        paste("Group", as.factor(letters[i])), rownames(iData)[i], sep = " : "
      )))
    }
  } else{
    for (i in 1:dim(data)[1]) {
      result <- rbind(result, data.frame(data[i, ] * transMat, group = paste(
        paste("Group", as.factor(i)), rownames(iData)[i], sep = " : "
      )))
    }
  }
  return(result)
...
generateCircle <- function(nLayer = NULL,
  ...
  circles <- data.frame(
    ...
    r = quantile(seq(
      0 + extendUnit, 1 + extendUnit, 1 / (nLayer - 1)
    ), seq(0, 1, 1 / (nLayer - 1)))
  )
  p <- ggplot() +
    ggforce::geom_circle(
...
generatePoint <- function(nPoly = NULL,
                          nLayer = NULL,
                          extendUnit = 0.5) {
  ...
  rList = quantile(seq(0 + extendUnit, 1 + extendUnit, 1 / (nLayer - 1)), seq(0, 1, 1 /
                                                                                (nLayer - 1)))
  degreeUnit <- round(360 / nPoly)
  allPoint <- data.frame(NULL)
  for (r in rList) {
    allPoint <- rbind(allPoint, data.frame(
      x = 0,
      y = r,
      cos = 0,
      sin = 1
...
getCutLine <- function(startPoint, len, perp) {
  ...
  result <-  c(perp[1] * adjust + startPoint[1] , perp[1] * adjust * (1 /
                                                                        r) + startPoint[2])
  return(result)
}
...
getCutDf <- function(tempDf, transMatrix, nP, indNum) {
  startDf <- data.frame(x = tempDf$cos,
                        y = tempDf$sin,
                        group = tempDf$group)
  perp1 <- lapply(
    1:nP,
    FUN = function(x)
      getPerpendicular(unlist(transMatrix[x, ]), changeSide = T)
  )
  perp2 <- lapply(
    1:nP,
    FUN = function(x)
      getPerpendicular(unlist(transMatrix[x, ]), changeSide = F)
  )
  ...
  colnames(cutDf1) <- c("x1", "y1", "obsGroup", "x2", "y2")
  colnames(cutDf2) <- c("x1", "y1", "obsGroup", "x2", "y2")
  
  cutDf1$x2 <- as.numeric(cutDf1$x2)
  cutDf1$y2 <- as.numeric(cutDf1$y2)
  cutDf2$x2 <- as.numeric(cutDf2$x2)
  cutDf2$y2 <- as.numeric(cutDf2$y2)
  return(list(data.frame(cutDf1), data.frame(cutDf2)))
}
build3DRect <- function(len = NULL,
                        wid = NULL,
                        hei = NULL,
                        g = NULL) {
  if (is.null(g)) {
  ...
  d <- vertice[, c(4, 5)]
  d <- rbind(d[c(1, 2, 4, 3), ], d[c(3, 4, 8, 7), ], d[c(5, 6, 8, 7), ], d[c(1, 2, 6, 5), ], d[c(1, 3, 7, 5), ], d[c(2, 4, 8, 6), ])
  group <- matrix(c(vertice[, 6], vertice[, 7], vertice[, 8], vertice[, 9], vertice[, 10], vertice[, 11]),
                  ncol = 1)
  group <- group[group != 0]
  d <- cbind(d, group)
...
shift <- function(data, unit) {
  unit <- 1 + unit
  data$x <- round(data$x, 10)
  data$y <- round(data$y, 10)
  data$x <- data$x * unit
  data$y <- data$y * unit
  return(data)
}
